...
var i = 0;
while (i < tvxq.length) {
  // 사용자에게 수정할 멤버 이름과 새로운 이름을 입력받기
  var userInput1 = prompt(`수정할 멤버 이름을 입력하세요.`);
  var userInput2 = prompt(`새로운 멤버 이름을 입력하세요.`);

  // 해당 멤버를 수정 / 수정 완료 후의 배열을 출력

  var target = `${userInput1}`;
  var index = tvxq.indexOf(target);

  if (tvxq.includes(userInput1)) {
    tvxq.splice(index, 1, `${userInput2}`);
    alert(`수정 완료!\n현재 멤버: ${tvxq}`);
    break;
  } else {
