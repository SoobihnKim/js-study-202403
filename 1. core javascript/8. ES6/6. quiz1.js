...
// 회원정보에서 회원의 첫번째 취미들만 다 추출해서 배열에 담아줘
const firstHobbyList = map(userList, user => user.hobbys[0]);
console.log(firstHobbyList);

// 회원정보에서 모든 회원의 이름과 계정을 연결해서 배열에 담아줘
// [대길이(abc1234), 빠나나(banana), ...]
const allNameAccount = map (userList, user => (`${user.userName}(${user.account})`));
console.log(allNameAccount);
