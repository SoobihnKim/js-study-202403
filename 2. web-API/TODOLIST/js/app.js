...
function toggleModifyMode ($modifyBtn) {
  ...
  //1. 수정모드 진입
  // const $targetModifyLi = e.target.closest(".todo-list-item").firstElementChild;
  // console.log($targetModifyLi);
  ...
  // 내 풀이
  // const changeClass = e.target;
  // console.log(changeClass);
  // changeClass.classList.replace('lnr-undo', 'lnr-checkmark-circle');

  //1-2. span.text를 input.modify-input으로 태그 교체
  // 부모태그.replaceChild(새로운태그, 없어질태그);
  ...
  //1-2-1. 교체된 input 태그 내부에는 기존 span의 텍스트가 그대로 들어가야함
...
//   const $targetModifyLi = e.target.closest(".todo-list-item");
//   // console.log($targetModifyLi);
//   modifyTodoData();
// });
...
//   const $targetcheckbox = e.target.closest(".todo-list-item");
